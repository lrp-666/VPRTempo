...
### S2.11 新学习规则探索（v6：B7 = Rank-BCM 规则，调研已完成）
...
**调研结论（三份独立 AI 调研的综合，全文见 `IDEA1-covstdp/docs/rule_synthesis_v1.md`）**：
- 团块根因三条件框架：输入去 1/f（PatchNorm 部分满足）+ 带符号表达（ON/OFF/free-sign 已满足）+ 高阶统计敏感或去相关（**当前缺的就是这个**）；
- (0.5−post) 是"去选择性"定点稳态器（三份一致），BCM 滑动阈值替换是理论最硬的改造；
- SoftHebb 接口干净但不解决方向选择性；triplet STDP 退化为 BCM 不用做；侧向 anti-Hebb 最对症但成本高，KH 名次反推是廉价替代。

**选定方案（B7 候选，Rank-BCM 规则）**：
```
ΔK_c = η · sign_rank · (θ_M,c − post) · (pre − 0.5) · Θ(post)
θ_M,c ← (1−α)·θ_M,c + α·post_c²     # 每通道 BCM 滑动阈值（α ≪ 权重时间常数 10–50×，防与 ITP 双回路振荡）
sign_rank：块内跨通道名次——winner +1，第 k 名 −δ（δ=0.4, k=2 起步）
```
保留不动：pre 中心化、保范数 L1、ITP、WTA、clamp、conv2d_weight 向量化路径。改动就两处手术，全部可开关可消融。

**详细操作**：
1. `conv_learning.py` 加 `bcm_theta`（每通道 EMA 状态）与 `rank_push`（块内跨通道名次反推）两个独立开关；
2. 迭代档三格：E1（仅 BCM 化）/ E2（仅名次反推）/ E3（B7 复合），对照 B2 0.878 / freesign 0.978 / B5 0.984（同口径 block2，已有）；
3. 预注册判据（见 synthesis 文档 §4）：B7 在符号约束 ON/OFF 下逼近 freesign 即最强叙事；每格过玩具测试断言 + 诊断 JSON（θ_M 曲线必查）；
4. 有效 → B7 进主表；无效 → 退回分析型叙事，负面结果本身可写。

**验收**：E1/E2/E3 迭代档数字 + 形态指标 + 诊断 JSON，回母会话判定 B7 是否进主表。
