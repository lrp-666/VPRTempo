...
**配置钉死**：主表里的 B2 ≡ R4 全配置（C=32, k=5, conv_epoch=2, WTA=local(2×2)（v4 修订，ADR-1）, agg=mean, pre_mode=centered, **ITP=on, E/I=on（ON/OFF 通路）, homeostasis=off**）；R2/R3 是阶梯专用独立配置名，不与 B2 混用，避免阶梯表与主表对不上。
...
【B2/B3 改动后】（维度数字以 28×28 输入、C=32, k=5, local WTA 2×2 主组合为例，依据 ADR-1）
RGB图 → ProcessImage(同上) → 展平 [1, 784]
      → 【新增】reshape 回 [1,1,28,28]
      → conv_layer_1: Conv2d(1→C1, k×k) + 阈值 + ON/OFF 通路 + WTA + STDP/ITP   ← 无监督训练后冻结
      → (B3: conv_layer_2: Conv2d(C1→C2, k×k), 同上)
      → 【ADR-1 强制】空间下采样：local WTA 的块 winner 图即 2×2 max-pool → [1,C,12,12]
      → flatten [1, 4608]
      → feature_layer(4608 → 9216) → output_layer(→500)
```
...
>
> **池化粒度拍板（v4，数据驱动）**：主组合 `wta_block` 从 4 改为 **2**。依据：阶段 2 变体扫描
> （results/s25_variant_sweep.md）发现池化粒度是唯一大杠杆——block2 轨 B 0.894 vs block4 0.772（500 地），
> 且只有 block2 在 3300 地让轨 A 转正（0.65 vs B0 0.59，results/s25_block2_gpu_scales.md）。
> 代价：flatten 维度 1152→4608，feature_layer 参数 2.65M→42.5M（35× B0，效率表如实报告）；
> GPU 上成本可接受（500 地训练 127.5s）。block∈{2,3,4} 进 S3.3 消融轴。
...
|---|---|---|---|
| 不下采样，直接 flatten | 32×24×24 = 18,432 | 18,432 × 36,864 ≈ 6.8×10⁸ 参数 ≈ **2.7 GB** | ❌ 不采用：存储在 24 GB 卡上勉强能跑，真正的否决理由是**参数量 ~560× 于 B0（1.2M），可比性破产** + `calc_stdp` 每样本 tile 同尺寸矩阵极慢 |
| 4×4 下采样（block4）后 flatten | 32×6×6 = 1,152 | 1,152 × 2,304 ≈ 2.65×10⁶ ≈ 10.6 MB | ⚠️ 曾是主组合；实测空间信息损失过大（轨 B 0.772，远低于 B0 的 0.948）——降为消融格 |
| **2×2 下采样（block2）后 flatten** | 32×12×12 = **4,608** | 4,608 × 9,216 ≈ 4.25×10⁷ ≈ 170 MB | ✅ **新主组合（v4）**：性能大杠杆；参数量 35× B0 须在效率表如实报告 |
...
**拍板**：主组合用 **(c)**——local WTA 的 winner 值重排后 flatten 送 feature_layer；WTA=none 的消融格子用 **(b)** 独立 max-pool 补足同一维度链（保证消融只变 WTA 一个变量）；WTA=(a) stride 不采用。**块边长从 4 修订为 2**（v4，数据驱动：block4 空间信息损失过大、block2 是变体扫描中唯一大杠杆且让 3300 地轨 A 转正；block3 未测，进消融轴）。

**WTA 模式与下游维度的耦合（必须写进论文设计说明；维度按 block2 主组合）**：
...
| local(2×2) | 块 winner 图 [C,12,12] flatten | 4,608 | 主组合（v4） |
| none | 全图 2×2 max-pool 后 flatten | 4,608 | 与 local 同维度，隔离 WTA 变量 |

**feature = 2×input 规则保留**（VPRTempoTrain.py:162）：下采样后 input=4,608 → feature=9,216，保持与 B0 相同的层宽比。
...
**向量化是硬性实现要求**：local WTA(2×2) 下每张图 winner 数 = C×(H'/2)×(W'/2) = 32×144 = **4,608 个**；Python 逐 winner 循环 × 1,000 图 × 2 epoch ≈ 920 万次迭代，不可行。
...
**内存对比（卖点素材，写给实验 1.5）**：全连接版每样本 tile 出 [in,out] = [4608, 9216] 全矩阵做更新；卷积版每样本只动 N_win×k² = 4608×25 个元素——**单样本更新量差 input 维度数量级**，这是"卷积 STDP 比全连接 STDP 便宜"的量化论据。
...
2. **特征提取点**（每个变体一个指定层，写进配置；**统一取池化后 flatten 向量（主组合 4,608 维）**——即 feature_layer 实际看到的那个向量，不是池化前的 18,432 维大图）：
...
**详细操作**：
1. 参数 `wta_mode ∈ {'global','local','none'}`，`wta_block`（主组合 = 2；4 为消融格，ADR-1 v4 修订）。
2. **global**：每通道 argmax，仅该位置保留原值，其余置零（flat argmax → scatter 成 mask → `z * mask`）。
3. **local**：不重叠 b×b 块（b=`wta_block`），每块一个 winner。实现用 reshape/unfold 分块 → 块内 argmax → 置零其余 → 拼回。H',W' 不被 b 整除时**裁掉右/下余数边缘**（简单、无 padding 伪影，论文注明）。主配置 28 输入 k=5 → H'=24，24/2=12 整除（block4 时 24/4=6 也整除）；仅 k=3（H'=26）等组合需要裁边。
...
2. 每个训练样本（**observed 必须在 WTA 之前的 clamp 图上统计**）：
   ...
   ```
   **为什么不能在 WTA 后统计**：local WTA 下每通道恒有 (H'/b)×(W'/b) 个 winner（block2 主组合 = 12×12 = 144 个），post-WTA 图上每通道发放率被结构钉死为常数 144/576 = 25%——`observed − f` 对所有通道同号同值，ITP 的差异化调节完全失效。global WTA 同理（恒 1 个 winner）。
...
2. **维度重算（照 ADR-1）**：`self.input = C × (H'/b) × (W'/b)`（主组合 = 32×12×12 = 4,608），由 dims/k/C/wta_block 参数在 `__init__` 算出，硬编码零容忍；feature_layer dims 随之改（VPRTempoTrain.py:161-162），feature = 2×input 规则保留。`--dims` 语义不变（仍是图像尺寸）。
...
2×2 池化（local WTA 块 winner 图）：24/2 = 12 → flatten = C2×12×12 = 32×144 = 4,608 ✓ 与 B2 严格同维
```

注意：两层都不 padding 的话是 28→24→20，池化后 10×10=100×C2=3,200 ≠ 4,608，B2/B3 下游维度就不可比了——**conv1 的 padding=2 是同维的关键**，不是可选项。**禁止先写代码后算维度**——feature_layer 参数量随 flatten 维度平方增长（ADR-1 的账）。类型分发天然支持多层（按 layer_dict 顺序各训各的）。
...
**设计修订（S2.9 实测发现，fork 后补记）**：纯冻结（同 S2.6，权重+阈值全冻）的 B5 轨 B R@1=0.660 < B1——根因不是 Gabor 覆盖，而是**工作点失配**：零均值带符号核是纯边缘响应（无直流承载），初始阈值 U[0,0.5] 下 25/32 通道死亡（发放率 1.6%）；非负随机核的直流响应恰好落在该阈值带是巧合。B2 无此问题正因 ITP=on。修订为 **frozen 权重 + ITP 阈值自适应**（`itp_on_frozen`）：STDP 被 frozen 守卫旁路、手工核逐位不变，ITP 只调阈值——对比变量仍是核的来源。修订后 B5 轨 B R@1=0.906（详见 results/s29_b5_gabor.md）。**推论**：B5 vs B1 由此混入第二变量（ITP），主表需补 B1+ITP 对照以干净隔离核来源。
...
### S2.11 新学习规则探索（v6：锦标赛制筛选 → B7 候选，调研已完成）

**背景与动机**：当前规则是 BLiTNet 单步幅度域规则的卷积化，本身无新规则贡献——审稿人会问"贡献是规则还是只是卷积化？"。若自提一个针对 VPR 的单步竞争可塑性规则且有效，贡献升级为"规则 + 系统 + 分析"三层。命名决策（S3.5：single-step competitive Hebbian）已为这条线铺好路。

**调研结论（三份独立 AI 调研的综合，全文见 `IDEA1-covstdp/docs/rule_synthesis_v1.md`）**：团块根因三条件框架；(0.5−post) 是"去选择性"定点稳态器；BCM 滑动阈值理论引用最硬。**但注意**：报告们对规则的优先级排序主要基于形态先验（能否学出条纹），而我们的 freesign 实测已证明**形态与性能解耦**——因此不做先验拍板，改用锦标赛筛选。

**锦标赛设计（迭代档：500 地 seed0 轨 B，GPU 每格 ~2min）**：

*Round 1 · 单机制变体*（每格只对 B2 主组合做一处手术，全部可开关）：

| 格 | 手术 | 机制来源 | 验证点 |
|---|---|---|---|
| R0 | —（B2 主组合 0.878，已有） | 基线 | 锚点 |
| R1 | 固定 0.5 → 每通道滑动阈值 θ_M（EMA of post²） | BCM | 自适应阈值的选择性锐化 |
| R2 | winner +1 / 块内跨通道第 k 名 −δ（δ=0.4, k=2） | KH 名次 | 廉价跨通道去相关 |
| R3 | 显式保范数 → Oja 衰减项（−post²·w） | Oja | 软归一化 vs 硬归一化 |
| R4 | pre 中心化 → (patch − kernel) 弹性项 | 竞争学习经典/SoftHebb 吸引子 | 重构式更新的形态影响 |
| R5 | 硬 WTA → soft-WTA（softmax 温度退火） | SoftHebb | 软竞争（Round 1 四个全败才做，成本最高） |

*Round 2 · 复合*：Round 1 赢家的两两/全组合（B7 候选在此产生）。
*Round 3 · 确认档*：Round 2 赢家跑 3300 地单 seed，防规模反转。

**预注册判据（写死再跑）**：
- 每格先过玩具测试断言（DC/AC 下降、核间余弦不升、范数稳定）再进真数据；
- Round 1 胜出标准：轨 B R@1 > R0 + 2 点（超迭代档噪声）；
- B7 终极判据：**符号约束 ON/OFF 下逼近 freesign（0.978）**——"规则设计弥补符号约束代价"即最强叙事；形态指标（R²/方向合成长度）只作机制证据，不作筛选标准；
- 每格留诊断 JSON（θ_M 曲线、发放率、DC/AC）；全部无效 → 退回分析型叙事，零损失。

**风险预案**：BCM 的 α 必须比权重学习慢 10–50 倍（防与 ITP 双反馈振荡，θ_M 初值 0.25=0.5² 与现规则不动点兼容）。

**验收**：Round 1 五格数字 + 诊断，回母会话判定 Round 2 组合。

---

## 5. 阶段 3：实验

### S3.2a 调参窗（v6 新增，主表前的调参步骤）

**背景**：目前所有超参均为继承值（blitnet 默认）或拍板结构值，**未做过系统调优**。block4→block2 后 winner 数 ×4，有效学习量已变，eta 大概率不再最优。

**详细操作**（全部迭代档：500 地单 seed 轨 B，贪心逐维扫，**不碰 3300 地与 3 seed**）：

| 旋钮 | 现值 | 扫描范围 | 预期收益 |
|---|---|---|---|
| `conv_epoch` | 2 | {2,4,8} | 高（B2 核 R² 低可能只是欠训） |
| `eta_stdp`（conv） | 0.005 | {×0.5, ×1, ×2} 对数扫 | 高（与 WTA 密度耦合） |
| `eta_ip`（conv） | 0.15 | {×0.5, ×1, ×2} | 高 |
| `thr_range` | [0,0.5] | {[0,0.3],[0,0.5],[0,0.7]} | 中（B5 死通道教训） |
| `fire_rate` 范围 | [0.2,0.9] | {[0.2,0.9],[0.3,0.8]} | 中 |
| 结构参数（k、C、block） | 已是消融轴 | 不调，扫完如实报告 | —（防混淆） |

调优对象默认 B2 主组合；freesign 若进主表，同协议扫一遍。**锁定后写入配置文件再进正式档；调参收益与规则收益在论文里分开声明。**

**验收**：锁定后的超参数写入配置；调参前后对照数字留档（results/）。
...
7. **free-sign 升级为主表正式行（v5 迭代档后，原 S3.3-8 消融格）**：`conv_stdp_freesign` 迭代档轨 B 0.978 ≈ B5，机制价值（符号约束是性能瓶颈）超出消融格量级，升主表。**thr 触底修复（fork G 已落地，2026-09-05）**：新增 `--conv_thr_min`（默认 0.0 = 原行为不变），`apply_itp_conv` 的 clamp 下界改取 `layer.thr_min`；freesign 主表行配置 `conv_thr_min=-0.5`，验证 100 步后 thr 分布不再贴 0（experiments/test_conv_thr_min.py 9/9）。**待办（母会话）**：B6b 是否以 `gabor_stdp_freesign + conv_thr_min=-0.5` 形式进主表仍待判定，不修复则只在消融里以"工作点崩塌"案例呈现。
...
**详细操作**：
1. **Table 2（WTA × 聚合 × 池化粒度）**：{global, local, none} × {mean, sum}，固定 C=32/k=5/B2，双轨；**另加 `wta_block ∈ {2,3,4}` 轴**（v4 新增：变体扫描显示池化粒度是唯一大杠杆，block3 未测，必须补上以支撑主组合 block2 的选择）。意义：WTA 稀疏度控制 STDP 样本效率，聚合方式控制更新尺度，块大小控制空间信息保留——三者都可能"差到不可用"，必须扫。
...
7. **编码探索（附录，可选）**：主实验一律用原始单步幅度编码（`SetImageAsSpikes`，像素/255），理由：①B0–B3 可比性，唯一变量是 conv 前端；②换多步编码（发放率/时延）会摧毁"无多步仿真"卖点——那是 SpikingJelly 参照行（S3.5）的领地。可选探索：**ON/OFF 双通道编码**（签名信号拆正/负两通道，类 DoG 中心-外周），与 conv-STDP 预期的中心-外周核结构天然契合；做则只跑 B2 主配置轨 B，作为附录一小节。注意它会改变输入通道数（1→2），不进主表。
...
## 5b. 阶段 4（候选方向，归档不插队）：任务对齐的局部可塑性

> 来源：2026-09-04 外部战略讨论。**进入条件：阶段 3 主表（S3.2）完成并判定之后，由母会话决定是否启动。**
> 核心诊断（成立）：现有规则优化的是"自然图像 patch 统计"——与 VPR 目标（跨条件不变性 + 地点间去相关）不对齐，B5 赢是结构性必然。要赢不能继续优化规则形式，要换目标。

**归档的四条改造（按性价比排序，启动时逐一评审）**：

| 编号 | 内容 | 成本 | 评估 |
|---|---|---|---|
| I3 | **读出探针表**：同特征 × 不同读出（cosine NN / spike forcing / 归一化 spike forcing / 线性探针），量化读出瓶颈 | 极低（现有模型不重训） | ✅ 优先启动项；把"创新点 2"从退路扶正为贡献 |
| I4 | 描述子几何：56×56 试点、多尺度池化、top-k 软 WTA、输入端 ON/OFF 编码 | 低（参数化改动） | ✅ 与 I3 同批；注意 56×56 需重跑 B0 基线（28×28 基线保留作主口径） |
| I1 | **时间迹/slowness 教师**：ΔK = η·(ỹ−θ_M)·(pre−x̄)·Θ(post)，ỹ 为跨帧 EMA 迹 | 中 | ⚠️ 两个未解设计问题：①相邻帧连续性只给单穿越内视角不变性，**跨季节不变性需显式构造对齐对**（filter/skip 会破坏对齐）；②与训练 shuffle=True 冲突（时序 vs 防时序记忆）。先 pilot 再投入 |
| I2 | 侧向 anti-Hebbian + ZCA 白化 | 中高 | ⏸ 等 I1/I3 结果后定 |

**配套评测改革**：选择集/报告集分离（超参在验证 traverse 上选）；跨条件为主战场（spring→winter 等）；指标对齐 LoCS-Net 口径（P@100%R）；第二数据集（ORC）从附录升格为必需。

**旗舰实验（阶段 4 的压轴）**：无标签在线适应——summer 训练 → winter 部署时仅用无标签 winter 图像在线更新前端，对照 B5（不可适应）与 B4（需标签+BP）。这是局部可塑性唯一不可替代的能力。

**止损**：若阶段 4 启动后跨条件上仍输 B5，诚实转分析型论文（机制链 + 双轨 + 读出瓶颈 + 形态/性能解耦已够 6 页）。
...
> **Gate 预演状态（单 seed，非正式判定）**：Gate 1 预演三规模全正（B2−B1 = +3.6/+5.6/+4.6 点，results/s25_b2_vs_b1_preview.md）；Gate 2 预演在 3300 地转正（block2：B2 轨 A 0.65 > B0 0.59，results/s25_block2_gpu_scales.md）。正式判定仍以 3-seed 主表为准——单 seed 趋势不能进论文。
>
> **v5 迭代档（block2 同口径，500 地 seed0，results/s210_b6_preview.md）——格局已变**：
> B5 0.984 ≈ **freesign（随机初始化+放开符号钳制）0.978** > B6a 0.936 ≫ B1 0.876 ≈ B2 0.878。
> 三个结构性发现：①**符号约束是性能瓶颈而非形态瓶颈**——freesign 从零学到 B5 级性能但核仍非条纹（R² 0.449），"符号约束⇒无条纹"假设被否，根因在规则不动点形态；②B6a < B5（−4.8 点）——在好先验上 STDP 微调轻微负增益（500 地，天花板效应待 3300 排除）；③**B5 双轨均超 B0**（轨 B 0.984>0.948、轨 A 0.980>0.93）——手工 Gabor 前端在 500 地也赢，"价值随规模增长"叙事需修正为"前端质量决定一切，符号约束下的学习无法企及手工先验"。
> **正式判定（2026-09-06，Table 1：8 变体 × 3 seeds × 双轨 × 双规模，results/table1_main.md）**：
> - **Gate 1（B2>B1 轨B）**：500 地 +1.7±1.1pt 勉强过；**3300 地 −1.2±1.1pt 不过**——从零学习在会议规模不优于随机核。但 B2+BCM−B1@3300 = +2.7pt（过）、B6a−B1@3300 = +6.9pt（过）——学习的价值依赖阈值机制与初始化。
> - **Gate 1.5（B6 vs B5）**：**不过**。B6a−B5 = −4.7pt（500）/ −8.3pt（3300）——在好先验上 STDP 微调是负增益。按预注册：转"手工前端 + 局部读出"为主线。
> - **Gate 2（轨A 3300 地 vs B0）**：B2 +0.3±0.6pt 平手（字面不过）；**但 B5 +12.0±0.8pt、B6a +4.3pt、B2+BCM +2.3±0.6pt 全过**——空间归纳偏置对完整系统有帮助，答案是肯定的，只是要靠手工先验或 BCM 化规则实现。
> - **意外发现**：freesign 的 500 地优势（0.978≈B5）在 3300 地**崩塌为全场最差 conv 变体**（轨 B 0.592 < B2 0.637）——小规模结论被规模反转的又一个实例，三档协议的价值自证。
> - **最终格局（3300 地轨 A R@1）**：B5 0.717 > B6a 0.640 > B2+BCM 0.620 > B2 0.600 ≈ B0 0.597 > freesign 0.567 > B1 0.530 > B1+ITP 0.433。
> - **选品（按 S3.2 第 8 条预注册判据）**：winner = **B5**（手工 Gabor 前端）与 **B6a**（Gabor 初始化+STDP）；学习路线代表 = **B2+BCM**。组合打磨候选：B5+BCM（冻结 Gabor + BCM 阈值替代 ITP）、B6a+BCM。
> - **外部对标**：B5 轨 B P@100%R@3300 = 0.802，超 LoCS-Net 报告的 78.6%（划分口径待核实后写入论文）。
>
> **动物园正式档（3 seeds，2026-09-07，results/table1_main.md 11 变体版）**：
> ①**B5+BCM 追平 B5**（3300 轨A 0.723±0.006 vs 0.717±0.006；轨B 0.801±0.006 平）且 **R@100%P 反超 +6.2pt**（0.448 vs 0.386）——BCM 滑动阈值作为冻结前端的阈值自适应机制 ≥ ITP，单 seed 发现坐实；
> ②**"频率覆盖 > 方向选择性"坐实**：DCT 基（有频率无方向）距 B5 仅 −2.3±0.6pt（3300 轨B R@1），Log-Gabor −2.6pt——方向性有贡献但小，带通频率覆盖解释了 Gabor 优势的绝大部分；
> ③诚实注脚：差距虽小但统计上非零（~3.8σ），论文措辞为"方向性提供小幅但真实的增量"，不要写成"无贡献"。
...
6. **算力规程（v4 新增）**：调参与正式实验默认走工作站 GPU（SSH 别名 `vpr-workspace`，操作细节与故障排查见本地 gitignored 文档 `IDEA1-covstdp/docs/workstation-handover.md`，**勿提交**——含内网地址）。关键纪律：非交互 SSH 每条命令前加 `export PATH=$HOME/.pixi/bin:$PATH`；长任务用 tmux；远程仓库分支保持与 origin 同步；GPU/CPU 结果一致性已验证（±1 点内，`results/s25_block2_gpu_scales.md`）。GitHub 网络抖动时用 `git -c http.lowSpeedLimit=1000 -c http.lowSpeedTime=30 push` 重试。
7. `papers/idea1-convstdp/` 是写作会话的 untracked 目录，任何 fork 都不得 add 进提交。
