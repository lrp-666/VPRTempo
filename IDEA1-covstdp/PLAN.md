...
**详细操作**：
1. **先调研后动手**：文献调研（Kimi Work 任务，提示词已在案）覆盖 Oja / BCM / Krotov-Hopfield / SoftHebb / triplet STDP / homeostatic 变体 / conv-STDP 各流派，产出候选规则评估报告；
2. 调研报告回母会话评审 → 选定 1–2 个候选 → 写新卡片（含公式、稳定性分析、与 WTA+ITP 的兼容性论证）再实现；
3. **复用现有测试基座**：新规则必须过同一套玩具测试断言（直流防线 DC/AC、去相关防线、范数稳定）+ 迭代档（500 地单 seed 轨 B）对照 B2/freesign；
4. 新规则有效 → 进主表作为本工作规则贡献（B7）；无效 → 现有规则的分析（机制证据）仍是论文内容，无损失。

**验收**：调研报告入库（`IDEA1-covstdp/docs/`）；若实现，玩具测试 + 迭代档数字。
